...
def test_load_ssl_config_no_verify():
    context = httpxr.create_ssl_context(verify=False)
...
def test_limits_repr():
    limits = httpxr.Limits(max_connections=100)
    expected = (
...
def test_limits_eq():
    limits = httpxr.Limits(max_connections=100)
    assert limits == httpxr.Limits(max_connections=100)


def test_timeout_eq():
    timeout = httpxr.Timeout(timeout=5.0)
    assert timeout == httpxr.Timeout(timeout=5.0)


def test_timeout_all_parameters_set():
    timeout = httpxr.Timeout(connect=5.0, read=5.0, write=5.0, pool=5.0)
    assert timeout == httpxr.Timeout(timeout=5.0)


def test_timeout_from_nothing():
    timeout = httpxr.Timeout(None)
...
def test_timeout_from_none():
    timeout = httpxr.Timeout(timeout=None)
    assert timeout == httpxr.Timeout(None)


def test_timeout_from_one_none_value():
    timeout = httpxr.Timeout(None, read=None)
    assert timeout == httpxr.Timeout(None)


def test_timeout_from_one_value():
    timeout = httpxr.Timeout(None, read=5.0)
    assert timeout == httpxr.Timeout(timeout=(None, 5.0, None, None))


def test_timeout_from_one_value_and_default():
    timeout = httpxr.Timeout(5.0, pool=60.0)
    assert timeout == httpxr.Timeout(timeout=(5.0, 5.0, 5.0, 60.0))
...
def test_timeout_missing_default():
    ...
    timeout = httpxr.Timeout(pool=60.0)
    assert timeout.pool == 60.0


def test_timeout_from_tuple():
    timeout = httpxr.Timeout(timeout=(5.0, 5.0, 5.0, 5.0))
    assert timeout == httpxr.Timeout(timeout=5.0)


def test_timeout_from_config_instance():
    timeout = httpxr.Timeout(timeout=5.0)
    assert httpxr.Timeout(timeout) == httpxr.Timeout(timeout=5.0)
...
def test_proxy_from_url():
    proxy = httpxr.Proxy("https://example.com")
...
def test_proxy_with_auth_from_url():
    proxy = httpxr.Proxy("https://username:password@example.com")
...
def test_invalid_proxy_scheme():
    with pytest.raises(ValueError):
        httpxr.Proxy("invalid://example.com")
