...
def test_status_code_as_int():
    ...
    assert httpxr.codes.NOT_FOUND == 404  # type: ignore[comparison-overlap]
    assert str(httpxr.codes.NOT_FOUND) == "404"


def test_status_code_value_lookup():
    assert httpxr.codes(404) == 404
...
def test_lowercase_status_code():
    assert httpxr.codes.not_found == 404  # type: ignore


def test_reason_phrase_for_status_code():
    assert httpxr.codes.get_reason_phrase(404) == "Not Found"


def test_reason_phrase_for_unknown_status_code():
    assert httpxr.codes.get_reason_phrase(499) == ""
