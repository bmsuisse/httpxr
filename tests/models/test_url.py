...
# Tests for `httpxr.URL` instantiation and property accessors.


def test_basic_url():
    url = httpxr.URL("https://www.example.com/")
...
def test_complete_url():
    url = httpxr.URL("https://example.org:123/path/to/somewhere?abc=123#anchor")
    assert url.scheme == "https"
...
def test_url_with_empty_query():
    ...
    """
    url = httpxr.URL("https://www.example.com/path")
...
def test_url_query_encoding():
    url = httpxr.URL("https://www.example.com/?a=b c&d=e/f")
    assert url.raw_path == b"/?a=b%20c&d=e/f"

    url = httpxr.URL("https://www.example.com/?a=b+c&d=e/f")
    assert url.raw_path == b"/?a=b+c&d=e/f"

    url = httpxr.URL("https://www.example.com/", params={"a": "b c", "d": "e/f"})
    assert url.raw_path == b"/?a=b+c&d=e%2Ff"


def test_url_params():
    url = httpxr.URL("https://example.org:123/path/to/somewhere", params={"a": "123"})
    assert str(url) == "https://example.org:123/path/to/somewhere?a=123"
    assert url.params == httpxr.QueryParams({"a": "123"})
...
def test_url_valid_host():
    url = httpxr.URL("https://example.com/")
    assert url.host == "example.com"
...
def test_url_valid_port():
    url = httpxr.URL("https://example.com:123/")
    assert url.port == 123


def test_url_normalized_port():
    # If the port matches the scheme default it is normalized to None.
    url = httpxr.URL("https://example.com:443/")
    assert url.port is None


def test_url_invalid_port():
    with pytest.raises(httpxr.InvalidURL) as exc:
        httpxr.URL("https://example.com:abc/")
...
def test_url_normalized_path():
    url = httpxr.URL("https://example.com/abc/def/../ghi/./jkl")
...
def test_url_leading_dot_prefix_on_absolute_url():
    url = httpxr.URL("https://example.com/../abc")
    assert url.path == "/abc"
...
def test_param_with_space():
    ...
    url = httpxr.URL("http://webservice", params={"u": "with spaces"})
...
def test_param_requires_encoding():
    ...
    url = httpxr.URL("http://webservice", params={"u": "%"})
...
def test_param_with_percent_encoded():
    ...
    url = httpxr.URL("http://webservice", params={"u": "with%20spaces"})
...
def test_param_with_existing_escape_requires_encoding():
    ...
    # even if they include a valid escape sequence.
    # We want to match browser form behaviour here.
    url = httpxr.URL("http://webservice", params={"u": "http://example.com?q=foo%2Fa"})
...
def test_query_with_existing_percent_encoding():
    ...
    url = httpxr.URL("http://webservice?u=phrase%20with%20spaces")
...
def test_query_requiring_percent_encoding():
    ...
    url = httpxr.URL("http://webservice?u=phrase with spaces")
    assert str(url) == "http://webservice?u=phrase%20with%20spaces"
...
def test_query_with_mixed_percent_encoding():
    ...
    url = httpxr.URL("http://webservice?u=phrase%20with spaces")
    assert str(url) == "http://webservice?u=phrase%20with%20spaces"
...
def test_url_invalid_hostname():
    """
    Ensure that invalid URLs raise an `httpxr.InvalidURL` exception.
    """
    with pytest.raises(httpxr.InvalidURL):
        httpxr.URL("https://😇/")


def test_url_excessively_long_url():
    with pytest.raises(httpxr.InvalidURL) as exc:
        httpxr.URL("https://www.example.com/" + "x" * 100_000)
    assert str(exc.value) == "URL too long"


def test_url_excessively_long_component():
    with pytest.raises(httpxr.InvalidURL) as exc:
        httpxr.URL("https://www.example.com", path="/" + "x" * 100_000)
    assert str(exc.value) == "URL component 'path' too long"


def test_url_non_printing_character_in_url():
    with pytest.raises(httpxr.InvalidURL) as exc:
        httpxr.URL("https://www.example.com/\n")
...
def test_url_non_printing_character_in_component():
    with pytest.raises(httpxr.InvalidURL) as exc:
        httpxr.URL("https://www.example.com", path="/\n")
    assert str(exc.value) == (
...
def test_url_with_components():
    url = httpxr.URL(scheme="https", host="www.example.com", path="/")
...
def test_url_with_relative_path():
    ...
    url = httpxr.URL(path="abc")
    assert url.path == "abc"


# Tests for `httpxr.URL` python built-in operators.


def test_url_eq_str():
    """
    Ensure that `httpxr.URL` supports the equality operator.
    """
    url = httpxr.URL("https://example.org:123/path/to/somewhere?abc=123#anchor")
...
def test_url_set():
    """
    Ensure that `httpxr.URL` instances can be used in sets.
    """
    urls = (
        httpxr.URL("http://example.org:123/path/to/somewhere"),
        httpxr.URL("http://example.org:123/path/to/somewhere/else"),
    )
...
# Tests for TypeErrors when instantiating `httpxr.URL`.


def test_url_invalid_type():
    """
    Ensure that invalid types on `httpxr.URL()` raise a `TypeError`.
    """
    ...
    with pytest.raises(TypeError):
        httpxr.URL(ExternalURLClass())  # type: ignore
...
def test_url_join():
    ...
    """
    url = httpxr.URL("https://example.org:123/path/to/somewhere")
...
def test_relative_url_join():
    url = httpxr.URL("/path/to/somewhere")
...
def test_url_join_rfc3986():
    ...
    url = httpxr.URL("http://example.com/b/c/d;p?q")
...
def test_url_copywith_authority_subcomponents():
    ...
    url = httpxr.URL("https://example.org")
...
def test_url_copywith_security():
    ...
    """
    with pytest.raises(httpxr.InvalidURL):
        httpxr.URL("https://u:p@[invalid!]//evilHost/path?t=w#tw")

    url = httpxr.URL("https://example.com/path?t=w#tw")
...
def test_url_set_param_manipulation():
    ...
    """
    url = httpxr.URL("https://example.org:123/?a=123")
...
def test_idna_url(given, idna, host, raw_host, scheme, port):
    url = httpxr.URL(given)
    assert url == httpxr.URL(idna)
...
def test_url_unescaped_idna_host():
    url = httpxr.URL("https://中国.icom.museum/")
    assert url.raw_host == b"xn--fiqs8s.icom.museum"


def test_url_escaped_idna_host():
    url = httpxr.URL("https://xn--fiqs8s.icom.museum/")
    assert url.raw_host == b"xn--fiqs8s.icom.museum"


def test_url_invalid_idna_host():
    with pytest.raises(httpxr.InvalidURL) as exc:
        httpxr.URL("https://☃.com/")
...
def test_url_valid_ipv4():
    url = httpxr.URL("https://1.2.3.4/")
    assert url.host == "1.2.3.4"


def test_url_invalid_ipv4():
    with pytest.raises(httpxr.InvalidURL) as exc:
        httpxr.URL("https://999.999.999.999/")
...
def test_ipv6_url():
    url = httpxr.URL("http://[::ffff:192.168.0.1]:5678/")
...
def test_url_valid_ipv6():
    url = httpxr.URL("https://[2001:db8::ff00:42:8329]/")
    assert url.host == "2001:db8::ff00:42:8329"


def test_url_invalid_ipv6():
    with pytest.raises(httpxr.InvalidURL) as exc:
        httpxr.URL("https://[2001]/")
...
def test_ipv6_url_from_raw_url(host):
    url = httpxr.URL(scheme="https", host=host, port=443, path="/")
...
def test_ipv6_url_copy_with_host(url_str, new_host):
    url = httpxr.URL(url_str).copy_with(host=new_host)
