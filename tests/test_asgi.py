...
async def test_asgi_transport():
    async with httpr.ASGITransport(app=hello_world) as transport:
        request = httpr.Request("GET", "http://www.example.com/")
...
async def test_asgi_transport_no_body():
    async with httpr.ASGITransport(app=echo_body) as transport:
        request = httpr.Request("GET", "http://www.example.com/")
        response = await transport.handle_async_request(request)
        await response.aread()
        assert response.status_code == 200
...
@pytest.mark.anyio
async def test_asgi():
    transport = httpr.ASGITransport(app=hello_world)
    async with httpr.AsyncClient(transport=transport) as client:
...
async def test_asgi_urlencoded_path():
    transport = httpr.ASGITransport(app=echo_path)
    async with httpr.AsyncClient(transport=transport) as client:
        url = httpr.URL("http://www.example.org/").copy_with(path="/user@example.org")
...
async def test_asgi_raw_path():
    transport = httpr.ASGITransport(app=echo_raw_path)
    async with httpr.AsyncClient(transport=transport) as client:
        url = httpr.URL("http://www.example.org/").copy_with(path="/user@example.org")
        response = await client.get(url)
...
async def test_asgi_raw_path_should_not_include_querystring_portion():
    ...
    transport = httpr.ASGITransport(app=echo_raw_path)
    async with httpr.AsyncClient(transport=transport) as client:
        url = httpr.URL("http://www.example.org/path?query")
        response = await client.get(url)
...
async def test_asgi_upload():
    transport = httpr.ASGITransport(app=echo_body)
    async with httpr.AsyncClient(transport=transport) as client:
...
async def test_asgi_headers():
    transport = httpr.ASGITransport(app=echo_headers)
    async with httpr.AsyncClient(transport=transport) as client:
        response = await client.get("http://www.example.org/")
...
async def test_asgi_exc():
    transport = httpr.ASGITransport(app=raise_exc)
    async with httpr.AsyncClient(transport=transport) as client:
...
async def test_asgi_exc_after_response():
    transport = httpr.ASGITransport(app=raise_exc_after_response)
    async with httpr.AsyncClient(transport=transport) as client:
        with pytest.raises(RuntimeError):
            await client.get("http://www.example.org/")
...
async def test_asgi_disconnect_after_response_complete():
    ...
    transport = httpr.ASGITransport(app=read_body)
    async with httpr.AsyncClient(transport=transport) as client:
        response = await client.post("http://www.example.org/", content=b"example")
...
async def test_asgi_exc_no_raise():
    transport = httpr.ASGITransport(app=raise_exc, raise_app_exceptions=False)
    async with httpr.AsyncClient(transport=transport) as client:
        response = await client.get("http://www.example.org/")
