...
pub fn path_safe() -> String {
    (0x20u8..0x7Fu8)
        .filter(|&i| !matches!(i, 0x20 | 0x22 | 0x23 | 0x3C | 0x3E | 0x3F | 0x60 | 0x7B | 0x7D))
        .map(|i| i as char)
        .collect()
}
...
impl ParseResult {
    ...
    pub fn copy_with_dict(&self, kwargs: Option<&std::collections::HashMap<String, Option<String>>>) -> PyResult<ParseResult> {
        let mut result = self.clone();
        if let Some(kw) = kwargs {
            for (key, value) in kw {
                match key.as_str() {
                    "scheme" => { if let Some(v) = value { result.scheme = v.clone(); } }
                    "userinfo" => { if let Some(v) = value { result.userinfo = v.clone(); } }
                    "host" => { if let Some(v) = value { result.host = v.clone(); } }
                    "port" => {
                        if let Some(v) = value {
                    ...
                    "path" => { if let Some(v) = value { result.path = v.clone(); } }
                    "query" => { result.query = value.clone(); }
                    "fragment" => { result.fragment = value.clone(); }
                    _ => {}
                }
            }
        }
        ...
        if let Some(kw) = kwargs {
            for (key, value) in kw.iter() {
                ...
                match key_str.as_str() {
                    "scheme" => { if !value.is_none() { result.scheme = value.extract()?; } }
                    "userinfo" => { if !value.is_none() { result.userinfo = value.extract()?; } }
                    "host" => { if !value.is_none() { result.host = value.extract()?; } }
                    "port" => {
                        if value.is_none() { result.port = None; }
                        else { result.port = Some(value.extract()?); }
                    }
                    "path" => { if !value.is_none() { result.path = value.extract()?; } }
                    "query" => {
                        if value.is_none() { result.query = None; }
                        else { result.query = Some(value.extract()?); }
                    }
                    "fragment" => {
                        if value.is_none() { result.fragment = None; }
                        else { result.fragment = Some(value.extract()?); }
                    }
                    _ => {}
                }
            }
...
fn normalize_port_value(port_str: &str, scheme: &str) -> PyResult<Option<u16>> {
    if port_str.is_empty() { return Ok(None); }
    match port_str.parse::<u16>() {
        Ok(port) => {
            if default_port(scheme) == Some(port) { Ok(None) } else { Ok(Some(port)) }
        }
        Err(_) => {
            Err(crate::exceptions::InvalidURL::new_err(format!("Invalid port: '{}'", port_str)))
        }
    }
}
...
pub fn validate_host(host: &str) -> PyResult<()> {
    ...
    if unicode_host.chars().any(|c| 
        (c >= '\u{2600}' && c <= '\u{26FF}') || // Misc Symbols (e.g. Snowman)
        (c >= '\u{1F000}' && c <= '\u{1FFFF}')  // Supplementary Private Use Area-A / Symbols / Main Emoji range
    ) {
         return Err(crate::exceptions::InvalidURL::new_err(format!("Invalid IDNA hostname: '{}'", unicode_host)));
    }
    Ok(())
}

pub fn urlparse_impl(url: &str, kwargs: Option<&std::collections::HashMap<String, Option<String>>>) -> PyResult<ParseResult> {
    if let Some(kw) = kwargs {
        if !kw.is_empty() {
             // ... handling kwargs ...
            let scheme = kw.get("scheme").and_then(|v| v.clone()).unwrap_or_default();
            let userinfo = kw.get("userinfo").and_then(|v| v.clone()).unwrap_or_default();
            ...
            if (scheme_lower == "http" || scheme_lower == "https") && host.is_empty() {
                 return Err(crate::exceptions::InvalidURL::new_err("Invalid URL: empty host"));
            }
            return Ok(ParseResult {
                scheme: scheme_lower, userinfo, host: host.to_lowercase(),
    ...
    if url_str.is_empty() {
        return Ok(ParseResult {
            scheme: String::new(), userinfo: String::new(), host: String::new(),
            ...
            if let Some(u) = repaired_url {
                ...
                if e == url::ParseError::RelativeUrlWithoutBase {
                     ...
                     let (url_no_frag, fragment) = if let Some(pos) = url_str.find('#') {
                        (&url_str[..pos], Some(quote(&url_str[pos + 1..], &frag_safe())))
                    } else {
                        (url_str, None)
                    };

                    let (url_no_query, query) = if let Some(pos) = url_no_frag.find('?') {
                        (&url_no_frag[..pos], Some(quote(&url_no_frag[pos + 1..], &query_safe())))
                    } else {
                        (url_no_frag, None)
                    };
                    ...
                    return Ok(ParseResult {
                        scheme: String::new(), userinfo: String::new(), host: String::new(),
                        port: None, path: normalized_path, query, fragment,
                    });
                }

                // Handle IdnaError or InvalidDomainCharacter for encoded hosts (e.g. spaces)
                if matches!(e, url::ParseError::IdnaError | url::ParseError::InvalidDomainCharacter) && (url_str.contains('%') || url_str.contains(' ')) {
                     ...
                     if let Some((scheme_part, rest)) = url_str.split_once("://") {
                         ...
                         let (host, port_opts) = if let Some(colon_pos) = host_port.rfind(':') {
                             ...
                             if let Ok(p) = host_port[colon_pos+1..].parse::<u16>() {
                                 (host_port[..colon_pos].to_string().replace(' ', "%20"), Some(p))
                             } else {
                         ...
                         return Ok(ParseResult {
                             scheme, userinfo, host, port: port_opts,
                             path: path.to_string(), query: query_opts, fragment: frag_opts,
                         });
                     }
                }
                
                let msg = match e {
                     url::ParseError::InvalidIpv4Address => format!("Invalid IPv4 address: '{}'", extract_host_from_invalid_url(url_str)),
                     url::ParseError::InvalidIpv6Address => format!("Invalid IPv6 address: '{}'", extract_host_from_invalid_url(url_str)),
                     url::ParseError::InvalidPort => format!("Invalid port: '{}'", extract_port_from_invalid_url(url_str)),
                     url::ParseError::EmptyHost => {
                         // httpx allows empty host
                         let scheme = url_str.split("://").next().unwrap_or("").to_string();
                         if !scheme.is_empty() {
                              return Ok(ParseResult {
                                    scheme, userinfo: String::new(), host: String::new(),
                                    port: None, path: String::new(), query: None, fragment: None,
                              });
                         }
    ...
    let scheme_str = if url_str.starts_with("://") { String::new() } else { parsed_url.scheme().to_string() };
    let scheme = scheme_str;
    let userinfo = format!("{}{}", 
        parsed_url.username(), 
        parsed_url.password().map(|p| format!(":{}", p)).unwrap_or_default()
    );
...
pub fn urlparse(url: String, kwargs: Option<&Bound<'_, PyDict>>) -> PyResult<ParseResult> {
    if let Some(kw) = kwargs {
        if !kw.is_empty() {
            ...
            for (key, value) in kw.iter() {
                let key_str: String = key.extract()?;
                let val: Option<String> = if value.is_none() { None } else { Some(value.extract()?) };
