#!/usr/bin/env python3
"""
Comprehensive HTTP library benchmark for httpr.

Compares httpr against: urllib3, aiohttp, httpx, rnet, niquests, ry,
...
ROUNDS = 100  # Number of repetitions per scenario
...
LIBRARY_COLORS: dict[str, str] = {
    "httpr": "#00E676",  # Bright green — our hero
    "httpx": "#FF5252",  # Red
...
def bench_httpr(base_url: str) -> list[BenchResult]:
    ...
    async def concurrent_bench() -> list[BenchResult]:
        ...
        async with httpr.AsyncClient(base_url=base_url) as client:
            # Warmup
            for _ in range(WARMUP_ROUNDS):
                await client.get("/")
