...
def _is_binary(content: bytes, content_type: str) -> bool:
    ...
    return (
        bool(ct) and not any(ct.startswith(t) for t in text_types)
    ) or b"\0" in content
...
def main(
    ...
    try:
        with _httpxr_mod.Client() as client:  # type: ignore[attr-defined]
            ...
            if download is not None:
                ...
                if use_rich:
                    ...
                    console.print(
                        f"[green]✓[/green] Downloaded [bold]"
                        f"{len(response.content):,}[/bold] "
                        f"bytes to [cyan]{download}[/cyan]"
                    )
                return
            ...
            if use_rich:
                ...
                if timing:
                    ...
                    server = (
                        f"Server: {elapsed.total_seconds() * 1000:.1f}ms  "
                        if elapsed
                        else ""
                    )
                    console.print()
        ...
        if use_rich:
            Console(stderr=True).print(  # type: ignore
